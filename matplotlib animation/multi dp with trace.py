...
class double_pendulum:
    ...
    def propagator(self,state):
        ...
        self.tracex.append(self.L1*np.sin(self.theta1)+self.L2 *np.sin(self.theta2))
        self.tracey.append(-self.L1*np.cos(self.theta1)-self.L2 *np.cos(self.theta2))
...
p3=double_pendulum(m1=10, m2=20, L1=3, L2=4, theta1=np.pi/6, theta2=np.pi, theta1_dot=-0.1, theta2_dot=0.1)
...
for i in range(no_of_pendulum):
    ax.plot(p[i].tracex,p[i].tracey, '--', lw=0.5)
