...
class double_pendulum:
    ...
    def equations(self,states,t):
        ...
        o1_dot=(self.m2*g*np.sin(t2)*np.cos(delta_t)-self.m2*np.sin(delta_t)*(self.L1*o1**2*np.cos(delta_t)+self.L2*o2**2)-(self.m1+self.m2)*g*np.sin(t1))/(self.L1*(self.m1+self.m2*(np.sin(delta_t))**2))-0.1*t1_dot
        
        o2_dot=((self.m1+self.m2)*(self.L1*o1**2*np.sin(delta_t)-g*np.sin(t2)+g*np.sin(t1)*np.cos(delta_t))+self.m2*self.L2*o2**2*np.sin(delta_t)*np.cos(delta_t))/(self.L1*(self.m1+self.m2*(np.sin(delta_t))**2))-0.1*t2_dot
...
plt.plot(t,p1.sol[0],'k-',label=r'$\theta_1$',lw=1)
plt.plot(t,p1.sol[1],'r-',label=r'$\theta_2$',lw=1)
plt.plot(t,p1.sol[2],'g--',label=r'$\omega_1$',lw=1)
plt.plot(t,p1.sol[3],'b--',label=r'$\omega_2$',lw=1)
plt.title("Dragged Double pendulum animation",fontname="Times New Roman",fontsize=14)
...
if(savegif==True):
    ...
    frames[0].save("D:\\Codes\\matplotlib animation\\dragged_double_pendulum_animation.gif", save_all=True, append_images=frames[1:], loop=0, duration=50)
