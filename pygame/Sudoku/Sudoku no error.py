...
WRONG_CELL_COLOR = (255, 0, 0)  # Color for wrong entries
FONT_COLOR = (0, 0, 0)
...
class Sudoku:
    def __init__(self):
        ...
        self.wrong_cells = set()  # Set to keep track of wrong cells
    ...
    def remove_numbers(self, board):
        ...
        squares = 81
        empties = 70  # adjust this number to control difficulty
    ...
    def draw_grid(self, screen):
        ...
        for i in range(9):
            for j in range(9):
                if self.board[i][j] != 0:
                    if (i, j) in self.wrong_cells:
                        value = self.font.render(str(self.board[i][j]), True, WRONG_CELL_COLOR)
                    else:
    ...
    def place_value(self, value):
        ...
        if self.board[row][col] == 0:
            if self.is_valid(self.board, value, (row, col)):
                self.board[row][col] = value
                self.wrong_cells.discard((row, col))  # Remove from wrong cells set if placed correctly
            else:
                self.wrong_cells.add((row, col))  # Add to wrong cells set if placed incorrectly
